...
export function writeCache<T = Record<string, unknown>>(
  ...
  try {
    ...
    // be a stale large payload from a wider $select) and retry once —
    // keeps other entities intact. CRITICAL: never call `clearAllCaches`
    // here — that destroys the just-written project headers slot when a
    // later step (e.g. lines) overflows quota, leaving the UI with empty
    // "isEmpty" state on next reload.
    ...
    try {
      ...
      // rows are still in React state for the current session; the user
      // sees them until reload. Surfacing a console.warn so the dev tools
      // network/storage tab makes the failure observable.
